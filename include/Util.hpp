...
class Util {
private:
	static dd_edge addMandatory(const dd_edge &emptyNode, const int N, FeatureVisitor &v,
			forest *mdd);
	...
	static void addSingleImplications(const int N, const dd_edge &emptyNode,
			FeatureVisitor &v, dd_edge &c, forest *mdd, dd_edge &startingNode);
	static void addCrossTreeConstraints(const FeatureVisitor v, const dd_edge emptyNode,
